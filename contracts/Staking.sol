...
contract XXTokenStaking{
  ...
  function UnstakeAndClaim(uint _stakeID) public returns (bool result) {
    ...
    require(_stakePeriodInHour >= 1,"Can't unstake before 1 week"); //1week 168 in hour
  ...
  function MontlyBurn () onlyOwner external returns (uint result) {
    ...
    uint _burnAmount;
    if (block.timestamp - _lastBurnDate >=  120) { ////for 30 days = 2592000
  ...
  function TotalActiveStakeAmount() external  view returns (uint result) {
    ...
    for(uint i = 0; i <  stakeBoxs.length; i++) {
       ...
       if(stakeBoxs[i].isActive == true){ _totalStaker += _totalStaker.add(1); }
   }
        return _totalStaker;
  }
